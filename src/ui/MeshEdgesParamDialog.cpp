...
MeshEdgesParamDialog::MeshEdgesParamDialog(wxWindow* parent)
	: FramelessModalPopup(parent, "Mesh Edges Parameters", wxSize(400, 250))
{
    ...
    // Mesh Edges Appearance section
    wxStaticBox* appearanceBox = new wxStaticBox(m_contentPanel, wxID_ANY, "Mesh Edges Appearance");
    ...
    m_colorPicker = new wxColourPickerCtrl(m_contentPanel, wxID_ANY, *wxBLUE);
...
wxColour MeshEdgesParamDialog::getEdgeColor() const {
	return m_colorPicker ? m_colorPicker->GetColour() : *wxBLUE;
}

double MeshEdgesParamDialog::getEdgeWidth() const {
	return m_edgeWidth ? m_edgeWidth->GetValue() : 1.0;
}

int MeshEdgesParamDialog::getEdgeStyle() const {
	return m_edgeStyle ? m_edgeStyle->GetSelection() : 0;
}

bool MeshEdgesParamDialog::getShowOnlyNew() const {
