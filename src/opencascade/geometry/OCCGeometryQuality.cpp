...
OCCGeometryQuality::OCCGeometryQuality()
    : m_renderingQuality(RenderingConfig::RenderingQuality::Medium)
    ...
    , m_shadowBias(0.001)
    , m_lightingModel(RenderingConfig::LightingModel::Phong)
    , m_roughness(0.5)
    , m_metallic(0.0)
    , m_fresnel(0.04)
...
void OCCGeometryQuality::setRenderingQuality(RenderingConfig::RenderingQuality quality)
{
    ...
    switch (quality) {
        case RenderingConfig::RenderingQuality::Low:
            m_tessellationLevel = 1;
            m_antiAliasingSamples = 0;
            break;
        case RenderingConfig::RenderingQuality::Medium:
            m_tessellationLevel = 2;
            m_antiAliasingSamples = 4;
            break;
...
void OCCGeometryQuality::updateFromRenderingConfig()
{
    ...
    m_renderingQuality = config.getRenderingQuality();
    m_shadowMode = config.getShadowMode();
    m_shadowIntensity = config.getShadowIntensity();
    m_lightingModel = config.getLightingModel();
