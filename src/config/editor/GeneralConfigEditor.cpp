...
GeneralConfigEditor::GeneralConfigEditor(wxWindow* parent, UnifiedConfigManager* configManager, const std::string& categoryId)
    : ConfigCategoryEditor(parent, configManager, categoryId)
{
    createUI();
}

GeneralConfigEditor::~GeneralConfigEditor() {
}

void GeneralConfigEditor::createUI() {
    wxBoxSizer* mainSizer = new wxBoxSizer(wxVERTICAL);
    SetSizer(mainSizer);
}

void GeneralConfigEditor::loadConfig() {
...
void GeneralConfigEditor::groupItemsBySection() {
    auto items = m_configManager->getItemsForCategory(m_categoryId);
