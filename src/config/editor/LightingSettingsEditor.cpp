...
LightingSettingsEditor::LightingSettingsEditor(wxWindow* parent, UnifiedConfigManager* configManager, const std::string& categoryId)
    : ConfigCategoryEditor(parent, configManager, categoryId)
    , m_notebook(nullptr)
    , m_environmentPage(nullptr)
    , m_lightsPage(nullptr)
    , m_presetsPage(nullptr)
    , m_ambientColorButton(nullptr)
    , m_ambientIntensitySlider(nullptr)
    , m_ambientIntensityLabel(nullptr)
    , m_lightsList(nullptr)
    , m_addLightButton(nullptr)
    , m_removeLightButton(nullptr)
    , m_lightNameText(nullptr)
    , m_lightTypeChoice(nullptr)
    , m_lightEnabledCheck(nullptr)
    , m_positionXSpin(nullptr)
    , m_positionYSpin(nullptr)
    , m_positionZSpin(nullptr)
    , m_directionXSpin(nullptr)
    , m_directionYSpin(nullptr)
    , m_directionZSpin(nullptr)
    , m_lightColorButton(nullptr)
    , m_lightIntensitySlider(nullptr)
    , m_lightIntensityLabel(nullptr)
    , m_currentPresetLabel(nullptr)
...
LightingSettingsEditor::~LightingSettingsEditor() {
}

void LightingSettingsEditor::createUI() {
...
void LightingSettingsEditor::createEnvironmentPage() {
    ...
    // Bind events
    m_ambientColorButton->Bind(wxEVT_BUTTON, &LightingSettingsEditor::onColorButtonClicked, this);
    m_ambientIntensitySlider->Bind(wxEVT_SLIDER, &LightingSettingsEditor::onEnvironmentPropertyChanged, this);
}

void LightingSettingsEditor::createLightsPage() {
    ...
    // Bind events
    m_lightsList->Bind(wxEVT_LISTBOX, &LightingSettingsEditor::onLightSelected, this);
    m_addLightButton->Bind(wxEVT_BUTTON, &LightingSettingsEditor::onAddLight, this);
    m_removeLightButton->Bind(wxEVT_BUTTON, &LightingSettingsEditor::onRemoveLight, this);
    m_lightColorButton->Bind(wxEVT_BUTTON, &LightingSettingsEditor::onColorButtonClicked, this);
    m_lightIntensitySlider->Bind(wxEVT_SLIDER, &LightingSettingsEditor::onLightPropertyChanged, this);
    
    // Bind property change events
    m_lightNameText->Bind(wxEVT_TEXT, &LightingSettingsEditor::onLightPropertyChanged, this);
    m_lightTypeChoice->Bind(wxEVT_CHOICE, &LightingSettingsEditor::onLightPropertyChanged, this);
    m_lightEnabledCheck->Bind(wxEVT_CHECKBOX, &LightingSettingsEditor::onLightPropertyChanged, this);
    m_positionXSpin->Bind(wxEVT_SPINCTRLDOUBLE, &LightingSettingsEditor::onLightPropertyChanged, this);
    m_positionYSpin->Bind(wxEVT_SPINCTRLDOUBLE, &LightingSettingsEditor::onLightPropertyChanged, this);
    m_positionZSpin->Bind(wxEVT_SPINCTRLDOUBLE, &LightingSettingsEditor::onLightPropertyChanged, this);
    m_directionXSpin->Bind(wxEVT_SPINCTRLDOUBLE, &LightingSettingsEditor::onLightPropertyChanged, this);
    m_directionYSpin->Bind(wxEVT_SPINCTRLDOUBLE, &LightingSettingsEditor::onLightPropertyChanged, this);
    m_directionZSpin->Bind(wxEVT_SPINCTRLDOUBLE, &LightingSettingsEditor::onLightPropertyChanged, this);
}

void LightingSettingsEditor::createPresetsPage() {
    ...
    freecadButton->SetBackgroundColour(wxColour(230, 255, 230));
    ...
    freecadButton->Bind(wxEVT_BUTTON, &LightingSettingsEditor::onFreeCADPreset, this);
    ...
    navcubeButton->SetBackgroundColour(wxColour(255, 230, 255));
    ...
    navcubeButton->Bind(wxEVT_BUTTON, &LightingSettingsEditor::onNavcubePreset, this);
...
void LightingSettingsEditor::loadConfig() {
...
void LightingSettingsEditor::saveConfig() {
    applySettings();
    m_config.saveToFile();
    if (m_changeCallback) {
        m_changeCallback();
...
void LightingSettingsEditor::resetConfig() {
    m_config.resetToDefaults();
    updateEnvironmentProperties();
    updateLightList();
}

void LightingSettingsEditor::updateEnvironmentProperties() {
...
void LightingSettingsEditor::updateLightProperties() {
    if (m_currentLightIndex >= 0 && m_currentLightIndex < static_cast<int>(m_tempLights.size())) {
        ...
        if (m_lightNameText) m_lightNameText->SetValue(light.name);
        
        // Set light type selection
        ...
        if (m_lightEnabledCheck) m_lightEnabledCheck->SetValue(light.enabled);
        if (m_positionXSpin) m_positionXSpin->SetValue(light.positionX);
        if (m_positionYSpin) m_positionYSpin->SetValue(light.positionY);
        if (m_positionZSpin) m_positionZSpin->SetValue(light.positionZ);
        if (m_directionXSpin) m_directionXSpin->SetValue(light.directionX);
        if (m_directionYSpin) m_directionYSpin->SetValue(light.directionY);
        if (m_directionZSpin) m_directionZSpin->SetValue(light.directionZ);
        
        updateColorButton(m_lightColorButton, light.color);
        if (m_lightIntensitySlider) {
            m_lightIntensitySlider->SetValue(static_cast<int>(light.intensity * 100));
        }
        if (m_lightIntensityLabel) {
            m_lightIntensityLabel->SetLabel(wxString::Format("Intensity: %.1f", light.intensity));
        }
    }
}

void LightingSettingsEditor::updateColorButton(wxButton* button, const Quantity_Color& color) {
    if (!button) return;
...
Quantity_Color LightingSettingsEditor::wxColourToQuantityColor(const wxColour& wxColor) const {
    return Quantity_Color(wxColor.Red() / 255.0, wxColor.Green() / 255.0, wxColor.Blue() / 255.0, Quantity_TOC_RGB);
}

wxColour LightingSettingsEditor::quantityColorToWxColour(const Quantity_Color& color) const {
    Standard_Real r, g, b;
...
void LightingSettingsEditor::onLightSelected(wxCommandEvent& event) {
    m_currentLightIndex = m_lightsList->GetSelection();
    updateLightProperties();
}

void LightingSettingsEditor::onAddLight(wxCommandEvent& event) {
...
void LightingSettingsEditor::onRemoveLight(wxCommandEvent& event) {
    if (m_currentLightIndex >= 0 && m_currentLightIndex < static_cast<int>(m_tempLights.size())) {
...
void LightingSettingsEditor::onLightPropertyChanged(wxCommandEvent& event) {
    if (m_currentLightIndex >= 0 && m_currentLightIndex < static_cast<int>(m_tempLights.size())) {
        auto& light = m_tempLights[m_currentLightIndex];
        
        if (m_lightNameText) light.name = m_lightNameText->GetValue().ToStdString();
        ...
        if (m_lightEnabledCheck) light.enabled = m_lightEnabledCheck->GetValue();
        if (m_positionXSpin) light.positionX = m_positionXSpin->GetValue();
        if (m_positionYSpin) light.positionY = m_positionYSpin->GetValue();
        if (m_positionZSpin) light.positionZ = m_positionZSpin->GetValue();
        if (m_directionXSpin) light.directionX = m_directionXSpin->GetValue();
        if (m_directionYSpin) light.directionY = m_directionYSpin->GetValue();
        if (m_directionZSpin) light.directionZ = m_directionZSpin->GetValue();
        ...
        if (m_lightsList && m_currentLightIndex >= 0) {
...
void LightingSettingsEditor::onEnvironmentPropertyChanged(wxCommandEvent& event) {
    if (m_ambientIntensitySlider && m_ambientIntensityLabel) {
...
void LightingSettingsEditor::onColorButtonClicked(wxCommandEvent& event) {
...
void LightingSettingsEditor::applyPresetAndUpdate(const std::string& presetName, const std::string& description) {
    // Apply the preset
